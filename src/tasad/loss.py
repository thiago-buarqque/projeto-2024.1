...
from src.tasad.utils.utilts_func import shape_equal
from src.tasad.utils import *
###
...
def ssim(img1, img2, window_size=11, window=None, size_average=True, full=False, val_range=None):
    ...
    if img1.shape[1]!=img2.shape[1]:
        if img1.shape[1]>img2.shape[1]:
            img2        =       shape_equal(img1, img2).to(img1.device)
        else:
            img1        =       shape_equal(img2, img1).to(img2.device)
...
class SSIM(torch.nn.Module):
    def __init__(self, gpu_id, window_size=11, size_average=True, val_range=None):
